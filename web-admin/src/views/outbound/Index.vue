<template>
  <div class="outbound">
    ...
    <a-modal v-model:visible="showForm" title="新建出库单" :width="700" @before-ok="handleSubmit" @cancel="resetForm">
      <a-form :model="form" layout="vertical">
        ...
        <a-form-item label="客户">
          <a-input v-model="form.customer" placeholder="请输入客户" />
        </a-form-item>
        <a-form-item label="出库商品">
          <div v-for="(item, index) in form.items" :key="index" class="item-row">
            <a-select v-model="item.productId" placeholder="选择商品" style="flex: 2" @change="onProductChange(item)">
              <a-option v-for="p in products" :key="p.id" :value="p.id">{{ p.name }} (库存: {{ p.stockQuantity }})</a-option>
            </a-select>
    ...
    <a-modal v-model:visible="showDetail" title="出库单详情" :width="600">
      <a-descriptions :column="2" v-if="currentOrder">
        ...
        <a-descriptions-item label="客户">{{ currentOrder.customer }}</a-descriptions-item>
        <a-descriptions-item label="总数量">{{ currentOrder.totalQuantity }}</a-descriptions-item>
        <a-descriptions-item label="总成本">¥{{ currentOrder.totalAmount }}</a-descriptions-item>
...
const form = reactive({
  ...
  customer: '',
  remark: '',
  items: [{ productId: null, productName: '', productCode: '', quantity: 1, costPrice: 0 }]
})
...
const columns = [
  ...
  { title: '成本', dataIndex: 'totalAmount', width: 100 },
...
const itemColumns = [
  ...
  { title: '成本单价', dataIndex: 'costPrice' }
]
...
const addItem = () => {
  form.items.push({ productId: null, productName: '', productCode: '', quantity: 1, costPrice: 0 })
}
...
const handleApprove = async (record) => {
  try {
    await outboundApi.updateStatus(record.id, 'completed')
...
const resetForm = () => {
  ...
  form.customer = ''
  form.remark = ''
  form.items = [{ productId: null, productName: '', productCode: '', quantity: 1, costPrice: 0 }]
}
