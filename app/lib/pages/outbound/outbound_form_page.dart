...
class _OutboundFormPageState extends State<OutboundFormPage> {
  ...
  Future<void> _submit() async {
    ...
    try {
      final success = await OutboundService().createOrder(
            ...
            child: ListView(
              ...
              children: [
                _buildSection('基本信息', [
                  ...
                  TextField(controller: _customerController, decoration: const InputDecoration(labelText: '客户信息', border: OutlineInputBorder())),
                ]),
                ...
                const SizedBox(height: 16),
                _buildSection('备注', [
                  TextField(controller: _remarkController, maxLines: 3, decoration: const InputDecoration(labelText: '备注信息', border: OutlineInputBorder())),
                ]),
              ],
            ),
          ),
                ...
                children: [
                  Row(
                    ...
                    children: [
                      ...
                      Text('$_totalQuantity 件  ¥${_totalAmount.toStringAsFixed(2)}', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Color(0xFFFF7D00))),
                    ],
                  ),
                  const SizedBox(height: 12),
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFFFF7D00)),
                      child: _isLoading ? const CircularProgressIndicator(color: Colors.white) : const Text('提交出库单'),
                    ),
                  ),
                ],
              ),
  ...
  Widget _buildWarehouseSelector() {
    return DropdownButtonFormField<Warehouse>(
      ...
      decoration: const InputDecoration(labelText: '出库仓库', border: OutlineInputBorder()),
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
          Icon(Icons.add, color: Color(0xFFFF7D00)),
          SizedBox(width: 8),
          Text('添加商品', style: TextStyle(color: Color(0xFFFF7D00), fontWeight: FontWeight.w500)),
        ]),
      ),
    );
  }
            ...
            Expanded(
              child: ListView.builder(
                ...
                itemBuilder: (_, i) {
                  final p = _products[i];
                  return ListTile(
                    leading: Container(width: 40, height: 40, decoration: BoxDecoration(color: const Color(0xFFFF7D00).withOpacity(0.1), borderRadius: BorderRadius.circular(8)), child: const Icon(Icons.inventory_2, color: Color(0xFFFF7D00))),
                    title: Text(p.name),
                    subtitle: Text('${p.code} | ¥${p.costPrice.toStringAsFixed(2)}'),
                    trailing: const Icon(Icons.add_circle_outline, color: Color(0xFFFF7D00)),
                    onTap: () {
                      _addItem(p);
                      Navigator.pop(ctx);
                    },
  ...
  Widget _buildItemCard(int index, OutboundItem item) {
    return Container(
      ...
      child: Row(
        children: [
          ...
          SizedBox(
            width: 70,
            child: Text('¥${item.amount.toStringAsFixed(2)}', style: const TextStyle(fontWeight: FontWeight.bold, color: Color(0xFFFF7D00)), textAlign: TextAlign.right),
          ),
